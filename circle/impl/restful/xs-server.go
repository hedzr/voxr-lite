...
func (d *daemonImpl) OnXsServerRun(cmd *cmdr.Command, args []string, stopCh, doneCh chan struct{}) (err error) {
	logrus.Infof("    Starting XS Server...")

	myXsServerCoreImpl = newXsService()
	xsServer = xs.New(myXsServerCoreImpl)

	// xsServer.SetRegistrarOkCallback(scheduler.RegistrarOkCallback)
	// xsServer.SetRegistrarChangesHandler(scheduler.RegistrarChangesHandler)
