...
build_pb () {
	# main provision entry
	pushd $CD/api >/dev/null
	...
	INC="-I/usr/local/include -I. -I$CD/api -I$GOPATH/src"

	#	 -I$GOPATH/src/github.com/grpc-ecosystem/grpc-gateway/third_party/googleapis"

	headline "protoc: generate gRPC proto..."
	headline "   using INC: $INC"
	# headline "protoc -I$GOPATH/src -I. --go_out=.. *.proto"

	for d in $(ls -d *); do
	  [ -d $d ] && ls $d/*.proto 2>/dev/null 1>&2 && {
		  # protoc -I$GOPATH/src -I. --go_out=.. $d/*.proto
		  pushd $d >/dev/null
		  headline "..IN $(pwd).."
		  protoc $INC --go_out=plugins=grpc:. *.proto
		  # echo "grpc-gateway.."
		  # protoc $INC --grpc-gateway_out=logtostderr=true:. *.proto
		  # protoc $INC --swagger_out=logtostderr=true:. *.proto
		  # echo "dart gen.."
		  # protoc $INC --dart_out=grpc:. *.proto
		  popd>/dev/null
	  }
	done

	echo "PWD: $(PWD) | CD: $CD"
	find . -type f -iname '*.dart' -exec sed -i -r "s#import 'github.com#import 'package:flutter_app\/github.com#" {} \;
	find . -type f -iname '*.dart-r' -delete

	FLUTTER_APP=${FLUTTER_APP:-$(dirname $CD)/flutter-app}
	# ls -la $FLUTTER_APP/lib
	[ -d $FLUTTER_APP/lib ] || ls -la ../ && {
		[ -d $FLUTTER_APP/lib/github.com/im/api ] || mkdir -p $FLUTTER_APP/lib/github.com/im/api
		rsync -acrztopg --delete --exclude=*.go --exclude=*.pbserver.dart --exclude=*.pbgrpc.dart --exclude=*.sh --exclude=*.md --exclude=*.dart-r --exclude=*.json --exclude=*.proto --include=*.dart . $FLUTTER_APP/lib/github.com/im/api/

		# F=github.com/golang/protobuf/ptypes/timestamp/timestamp.pb.dart
		# D=$(dirname $F)
		# [ -d $FLUTTER_APP/lib/$D ] || mkdir -p $FLUTTER_APP/lib/$D
		# echo cp $GOPATH/src/$F $FLUTTER_APP/lib/$D/
		# cp $GOPATH/src/$F $FLUTTER_APP/lib/$D/
	}
...
build_pb_clean () {
	pushd $CD/api >/dev/null
	for x in .pb.go .pb.gw.go .dart .json; do
	  rm -f v10/*$x
	done
	popd >/dev/null
}
